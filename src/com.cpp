...
int Com_HashKey(char *string, int maxlen) {
	//int register hash, i;
	//jampio - use of register storage specifier seems nonstandard
	int hash, i;
...
=================
*/
static void Com_Crash_f(CommandArgs& args) {
	* ( int * ) 0 = 0x12345678;
}
...
void Com_WriteConfigToFile(CvarSystem& cvars, const char *filename) {
#ifndef _XBOX
	fileHandle_t	f;
...
===============
*/
void Com_WriteConfiguration(CvarSystem& cvars) {
	// if we are quiting without fully initializing, make sure
	// we don't write out anything
	if (!com_fullyInitialized) {
		return;
	}
	if (!cvars.hasModifiedFlags(CVAR_ARCHIVE)) {
		return;
	}
	cvars.removeModifiedFlags(CVAR_ARCHIVE);

#ifdef DEDICATED
	Com_WriteConfigToFile(cvars, "jampserver.cfg");
#else
	Com_WriteConfigToFile(cvars, "jampconfig.cfg");
#endif

	// bk001119 - tentative "not needed for dedicated"
#ifndef DEDICATED
#ifdef USE_CD_KEY
	Cvar& fs = cvars.Get("fs_game", "", CVAR_INIT|CVAR_SYSTEMINFO);
	if (UI_usesUniqueCDKey() && fs.string()[0] != 0) {
...
===============
*/
void Com_WriteConfig_f(CvarSystem& cvars, CommandArgs& args) {
	...
	Q_strncpyz(filename, args.Argv(1), sizeof(filename));
	COM_DefaultExtension(filename, sizeof(filename), ".cfg");
	Com_Printf("Writing %s.\n", filename);
	Com_WriteConfigToFile(cvars, filename);
}
...
void Com_Init(CvarSystem& cvars, CommandLine& cli, CommandBuffer& cbuf, CommandSystem& cmd) {
	...
	try
	{
		...
		FS_InitFilesystem(cmd, cvars, cli);

		Com_InitJournaling(cvars, cli);

		cbuf.AddText("exec mpdefault.cfg\n");

		// skip the jampconfig.cfg if "safe" is on the command line
		if ( !cli.SafeMode() ) {
#ifdef DEDICATED
			cbuf.AddText("exec jampserver.cfg\n");
#else
			cbuf.AddText("exec jampconfig.cfg\n");
#endif
		}
	  ...
	  // get dedicated here for proper hunk megs initialization
	#ifdef DEDICATED
		com_dedicated = cvars.Get("dedicated", "2", CVAR_ROM);
	#else
		com_dedicated = cvars.Get("dedicated", "0", CVAR_LATCH);
	#endif
		...
		//
		com_maxfps = cvars.Get("com_maxfps", "85", CVAR_ARCHIVE);
		com_blood = cvars.Get("com_blood", "1", CVAR_ARCHIVE);
		com_developer = cvars.Get("developer", "0", CVAR_TEMP);
		com_vmdebug = cvars.Get("vmdebug", "0", CVAR_TEMP);
		com_logfile = cvars.Get("logfile", "0", CVAR_TEMP);
		com_timescale = cvars.Get("timescale", "1", CVAR_CHEAT | CVAR_SYSTEMINFO);
		com_fixedtime = cvars.Get("fixedtime", "0", CVAR_CHEAT);
		com_showtrace = cvars.Get("com_showtrace", "0", CVAR_CHEAT);
		com_terrainPhysics = cvars.Get("com_terrainPhysics", "1", CVAR_CHEAT);
		com_dropsim = cvars.Get("com_dropsim", "0", CVAR_CHEAT);
		com_viewlog = cvars.Get( "viewlog", "0", CVAR_CHEAT);
		com_speeds = cvars.Get("com_speeds", "0", 0);
		com_timedemo = cvars.Get("timedemo", "0", 0);
		com_cameraMode = cvars.Get("com_cameraMode", "0", CVAR_CHEAT);
		com_optvehtrace = cvars.Get("com_optvehtrace", "0", 0);
		cl_paused = cvars.Get("cl_paused", "0", CVAR_ROM);
		sv_paused = cvars.Get("sv_paused", "0", CVAR_ROM);
		com_sv_running = cvars.Get("sv_running", "0", CVAR_ROM);
		com_cl_running = cvars.Get("cl_running", "0", CVAR_ROM);
		com_buildScript = cvars.Get("com_buildScript", "0", 0 );

#ifdef G2_PERFORMANCE_ANALYSIS
		com_G2Report = cvars.Get("com_G2Report", "0", 0);
#endif

		com_RMG = cvars.Get("RMG", "0", 0);

		cvars.Get("RMG_seed", "0", 0);
		cvars.Get("RMG_time", "day", 0);
		cvars.Get("RMG_soundset", "", 0);
		cvars.Get("RMG_textseed", "0", CVAR_SYSTEMINFO|CVAR_ARCHIVE);
		cvars.Get("RMG_map", "small", CVAR_ARCHIVE|CVAR_SYSTEMINFO);
		cvars.Get("RMG_timefile", "day", CVAR_ARCHIVE);
		cvars.Get("RMG_terrain", "grassyhills", CVAR_ARCHIVE);
		cvars.Get("RMG_sky", "", CVAR_SYSTEMINFO);
		cvars.Get("RMG_fog", "", CVAR_SYSTEMINFO);
		cvars.Get("RMG_weather", "", CVAR_SYSTEMINFO|CVAR_SERVERINFO|CVAR_CHEAT);
		cvars.Get("RMG_instances", "colombia", CVAR_SYSTEMINFO);
		cvars.Get("RMG_miscents", "deciduous", 0);
		cvars.Get("RMG_music", "music/dm_kam1", 0);
		cvars.Get("RMG_mission", "ctf", CVAR_SYSTEMINFO);
		cvars.Get("RMG_course", "standard", CVAR_SYSTEMINFO);
		cvars.Get("RMG_distancecull", "5000", CVAR_CHEAT);

		com_introPlayed = cvars.Get( "com_introplayed", "0", CVAR_ARCHIVE);
		...
		if (com_developer && com_developer->integer()) {
			cmd.AddCommand("error", Com_Error_f);
			cmd.AddCommand("crash", Com_Crash_f );
		...
		cmd.AddCommand("writeconfig", [&](auto& args) {
			Com_WriteConfig_f(cvars, args);
		});
		...
		com_dedicated->setModified(false);
		if (!com_dedicated->integer()) {
			CL_Init();
		...
		if (!cli.AddStartupCommands()) 
		{
			...
			{
#ifndef _DEBUG
				cbuf.AddText("cinematic openinglogos.roq\n");
#endif
				// intro.roq is iD's.
...
//				}
		...
		cvars.Set("ui_singlePlayerActive", "0");
