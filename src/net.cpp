...
===============
*/
void Netchan_Init(CvarSystem& cvars, int port) {
	port &= 0xffff;
	showpackets = cvars.Get ("showpackets", "0", CVAR_TEMP );
	showdrop = cvars.Get ("showdrop", "0", CVAR_TEMP );
	qport = cvars.Get ("net_qport", va("%i", port), CVAR_INIT );
	net_killdroppedfragments = cvars.Get ("net_killdroppedfragments", "0", CVAR_TEMP);
}
