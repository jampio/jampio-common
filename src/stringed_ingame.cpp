...
Cvar *se_language = NULL;
Cvar *se_debug = NULL;
Cvar *sp_leet = NULL;	// kept as 'sp_' for JK2 compat.
...
typedef std::map <string, SE_Entry_t> mapStringEntries_t;
...
public:
	...
	// flag stuff...
	//
	std::vector <string>		m_vstrFlagNames;
	std::map	<string,int>	m_mapFlagMasks;
...
{
	std::map <string, int>::iterator itFlag = m_mapFlagMasks.find( psFlagName );
...
LPCSTR Leetify( LPCSTR psString )
{
	...
	if (sp_leet->integer() == 42)	// very specific test, so you won't hit it accidentally
	{
		...
		{
			while ((p=strchr(str.data(),cReplace[i]))!=NULL)
				*p = cReplace[i+1];
		}
	}
...
//
std::vector <string> gvLanguagesAvailable;
int SE_GetNumLanguages(void)
{
	if ( gvLanguagesAvailable.empty() )
	{
		...
		std::set<string> strUniqueStrings;	// laziness <g>
		LPCSTR p;
...
//	int iNumLanguages = SE_GetNumLanguages();
	...
	se_language = cvars.Get("se_language", "english", CVAR_ARCHIVE | CVAR_NORESTART);
	se_debug = cvars.Get("se_debug", "0", 0);
	sp_leet = cvars.Get("sp_leet", "0", CVAR_ROM );
