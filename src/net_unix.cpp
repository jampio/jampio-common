...
void NET_OpenIP(CvarSystem& cvars) {
	auto& ip = cvars.Get("net_ip", "localhost", 0);
	auto port = cvars.Get("net_port", va("%i", PORT_SERVER), 0).integer();

	for (int i = 0 ; i < 10 ; i++) {
		ip_socket = NET_IPSocket(ip.string(), port + i);
		if (ip_socket) {
			cvars.Set("net_port", port + i);
			NET_GetLocalAddress();
			return;
		}
	}
...
====================
*/
void NET_Init(CvarSystem& cvars) {
	noudp = cvars.Get("net_noudp", "0", 0);
	// open sockets
	if (!noudp->integer()) {
		NET_OpenIP(cvars);
	}
