...
int FS_FOpenFileRead( const char *filename, fileHandle_t *file, qboolean uniqueFILE ) {
	...
	*file = FS_HandleForFile();
			...
			if ( search->pack && search->pack->hashTable[hash] ) {
				...
				do {
					...
					if ( !FS_FilenameCompare( pakFile->name, filename ) ) {
						...
						/*
						FS_ShiftedStrStr(filename, "jk2mpgamex86.dll", -13);
												  //]^%`cZT`Xk+)!W__
						...
						*/
						...
						{
							if (FS_ShiftedStrStr(filename, "]^%`cZT`X!di`", 13) ||
								FS_ShiftedStrStr(filename, "]^%`cZT`Xk+)!W__", 13))
							{
						...
						#if 0 // jampio - unzFile struct size is untyped & unknowable
	...
	#ifndef DEDICATED
	#ifndef FINAL_BUILD
	#if 0 // jampio - leaving out for now, reduce coupling
						...
						unz_file_info fi;
						if (unzGetCurrentFileInfo(fsh[*file].handleFiles.file.z, &fi, NULL, 0, NULL, 0, NULL, 0) == UNZ_OK) {
							return fi.uncompressed_size;
						} else {
							Com_Printf(S_COLOR_YELLOW "WARNING: unzGetCurrentFileInfo failed\n");
							return 0;
						}
					}
	...
	#ifndef DEDICATED
	#ifndef FINAL_BUILD
	#if 0 // jampio - leaving out for now, reducing coupling
				// Check for unprecached files when in game but not in the menus
				if((cls.state == CA_ACTIVE) && !(cls.keyCatchers & KEYCATCH_UI))
				{
					Com_Printf(S_COLOR_YELLOW "WARNING: File %s not precached\n", filename);
